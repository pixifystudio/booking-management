...
if (isset($_GET['id'])) {
    ...
    $mySqlCek  = "SELECT paket FROM master_jenis WHERE  id ='" . $_GET['id'] . "'";
	...
	VALUES ('master_jenis','" . $_GET['id'] . "','" . $_SESSION['SES_NAMA'] . "', NOW())";
    $myQry1 = mysqli_query($koneksidb, $mySql1) or die("RENTAS ERP ERROR : " . mysqli_error($koneksidb));

    $mySql = "DELETE FROM master_jenis WHERE id='" . $_GET['id'] . "'";
    $myQry = mysqli_query($koneksidb, $mySql) or die("RENTAS ERP ERROR : " . mysqli_error($koneksidb));

    // delete anak anak nya
    $mySql = "DELETE FROM master_background WHERE jenis='$dataPaketOld'";
    $myQry = mysqli_query($koneksidb, $mySql) or die("RENTAS ERP ERROR : " . mysqli_error($koneksidb));
    if ($myQry) {
        // Refresh halaman
        echo "<meta http-equiv='refresh' content='0; url=?page=Master-Paket&s=deleted'>";
    }
} else {
