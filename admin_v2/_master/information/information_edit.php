...
$type = $myData['type'];
$writer = $myData['writer'];
$title_in = $myData['title_in'];
$title_en = $myData['title_en'];
$description_in = $myData['description_in'];
...
if (isset($_POST['btnSubmit'])) {
  ...
  $txtHeadline = isset($_POST['txtHeadline']) ? $_POST['txtHeadline'] : '';
  ...
  if (count($pesanError) >= 1) {
    ...
    try {
      ...
      if ($txtHeadline == 1) {
        $qryheadline = ", headline = '1'";

        $mySql   = "SELECT * FROM information  where headline='1'";
        $myQry   = mysqli_query($koneksidb, $mySql)  or die("Error query " . mysqli_error($koneksidb));
        $nomor  = 0;
        $myData = mysqli_fetch_array($myQry);
        $idInfo =  $myData['id'];
        $mySql    = "UPDATE information SET headline =''
						WHERE id = '".$idInfo."' ";
        $myQry = mysqli_query($koneksidb, $mySql) or die("Error query " . mysqli_error($koneksidb));
      }
      ...
      $mySql    = "UPDATE information SET type='$txtType', writer='$txtWriter',title_in='$txtTitleIN',title_en='$txtTitleEN',
						description_in='$txtBodyIN',description_en ='$txtBodyEN', source='$txtSource', file='$file_cover', status='Active',
            updated_by='$ses_nama'  , updated_date=now() $qryheadline
						WHERE id = '$ID'";
      $myQry = mysqli_query($koneksidb, $mySql) or die("Error query " . mysqli_error($koneksidb));
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="mb-1">
              ...
              <select name="txtType" class="form-control" tabindex="-1">
                <?php
                $mySql = "SELECT status_name FROM master_status where status_group ='Type' ";
                $dataQry = mysqli_query($koneksidb, $mySql) or die("Eror Query" . mysqli_error($koneksidb));
                while ($dataRow = mysqli_fetch_array($dataQry)) {
                  if ($dataRow['status_name'] == $type) {
                    $cek = " selected";
                  } else {
                    $cek = "";
                  }
            ...
            <div class="mb-1">
              ...
              <input type="text" name="txtWriter" id="basic-addon-name" value="<?php echo $writer ?>" class="form-control" placeholder="Name" aria-label="Name" aria-describedby="basic-addon-name" required />
            </div>
            <h2>Content</h2>
            ...
            <div class="mb-1">
              ...
              <input type="text" name="txtTitleIN" id="basic-addon-name" class="form-control" value="<?php echo $title_in ?>" placeholder="Title" aria-label="Name" aria-describedby="basic-addon-name" required />
            </div>
            <div class="mb-1">
              <label class="d-block form-label" for="validationBioBootstrap">Body</label>
              <textarea class="form-control" id="validationBioBootstrap" name="txtBodyIN" rows="3" required><?php echo $description_in ?></textarea>
            </div>
            ...
            <div class="mb-1">
              ...
              <input type="text" name="txtTitleEN" id="basic-addon-name" class="form-control" placeholder="Title" value="<?php echo $title_en ?>" aria-label="Name" aria-describedby="basic-addon-name" required />
            </div>

            <div class="mb-1">
              <label class="d-block form-label" for="validationBioBootstrap">Body</label>
              <textarea class="form-control" id="validationBioBootstrap" name="txtBodyEN" rows="3" required><?php echo $description_en ?></textarea>
            </div>
            <div class="mb-1">
              <label class="form-label" for="basic-addon-name">Source </label>

              <input type="text" name="txtSource" id="basic-addon-name" class="form-control" placeholder="Source" value="<?php echo $source ?>" aria-label="Name" aria-describedby="basic-addon-name" required />
            </div>
            <div class="mb-1">
              <label class="form-label" for="basic-addon-name">Edit Photo </label>
              <input type="file" id="basic-addon-name" name="txtFileCover" class="form-control" placeholder="Name" aria-label="Name" aria-describedby="basic-addon-name" />
              <span> <i>*Recommended Resolution 720px x 360px</i> </span>

            </div>
            <div class="mb-1">
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="checkbox" id="inlineCheckbox1" name='txtHeadline' value="checked"/>
                <label class="form-check-label" for="inlineCheckbox1">Set as Headline</label>
              </div>
            </div>
            <!-- <div class="mb-1">
