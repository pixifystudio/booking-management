...
if (isset($_GET['id'])) {
   ...
   VALUES ('master_reward
   ','" . $_GET['id'] . "','" . $_SESSION['SES_NAMA'] . "', NOW())";
   $myQry1 = mysqli_query($koneksidb, $mySql1) or die("Eror hapus data" . mysqli_error($koneksidb));

   $mySql = "DELETE FROM master_reward WHERE  id='" . $_GET['id'] . "'";
   $myQry = mysqli_query($koneksidb, $mySql) or die("Eror hapus data" . mysqli_error($koneksidb));

   if ($myQry) {
      // Refresh halaman
      echo "<meta http-equiv='refresh' content='0; url=?page=Management-Reward'>";
   }
} else {
