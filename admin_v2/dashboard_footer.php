...
// top 5 total document
<?php 
$datafile=array();
$datatotal=array();
$mySql 	= "select document_title_id, total from view_log_document_top10 order by total desc limit 5";
$myQry 	= mysqli_query($koneksidb,$mySql)  or die ("Error query ".mysqli_error($koneksidb));
while ($myData = mysqli_fetch_array($myQry)) {
	$datadoc[]=$myData['document_title_id'];
	$datatotald[]=$myData['total'];
}
?>
	...
	// horizontal bar
      new Chart(document.getElementById("bar-chart-horizontal-2"), {
		type: 'horizontalBar',
		data: {
		  labels: [<?php foreach ($datadoc as $doc) {echo '"'.substr($doc,0,30).'",'; } ?>],
		  datasets: [
			{
			  label: "Total dibaca",
			  backgroundColor: ["#146555","#1A826C","#1F9C83","#24B89A","#41DABB","#146555","#1A826C","#1F9C83","#24B89A","#41DABB"],
			  data: [<?php foreach ($datatotald as $total) {echo number_format($total) . ','; } ?>]
			}
		  ]
		},
