    ...
    if (count($pesanError) >= 1) {
      ...
      try {
        ...
        if ($_FILES['txtFileBook']['name'] != "") {
          ...
          $fileTitle = str_replace("&", "And", $dataCode . '.pdf');



          $targetDir = "../document_index/";
          $fileName = str_replace('&', 'And', $dataCode . '.pdf');
          ...
          if (move_uploaded_file($_FILES['txtFileBook']['tmp_name'], $targetFile)) {
            ...
            if ($dataStatusOld  == "Created" || $dataStatusOld  == "Updated" || $dataStatusOld  == "Revised") {
              $mySql    = "DELETE FROM document_files where document_id='$txtCode' AND document_version='$txtVersion'";
              $myQry = mysqli_query($koneksidb, $mySql);
              if (!$myQry)
                throw new Exception("Form gagal diinput. code:f1. " . mysqli_error($koneksidb));

              $mySql    = "DELETE FROM document_index where document_id='$txtCode' AND document_version='$txtVersion'";
              $myQry = mysqli_query($koneksidb, $mySql);
              if (!$myQry)
                throw new Exception("Form gagal diinput. code:f1. " . mysqli_error($koneksidb));
            }
        ...
        if ($dataStatusOld  == "Created" || $dataStatusOld  == "Updated" || $dataStatusOld  == "Revised") {
          ...
          $mySql = "INSERT INTO document_archive (
            SELECT document_id, category_id, document_version, document_date, document_expire_date, document_status, document_keyword, document_title_id, document_title_en, document_description_id, document_description_en, document_year, racking_id, document_publisher, document_author, document_cover, document_change_history, document_note, updated_by, updated_date FROM document WHERE document_id='$getID' AND document_version='$getVersion')";
          $myQry = mysqli_query($koneksidb, $mySql);
          if (!$myQry)
            throw new Exception("Form gagal diinput. code:d2. " . mysqli_error($koneksidb));

          $mySql    = "INSERT INTO document (document_year, document_publisher, document_author, document_cover, 
          ...
          document_change_history, updated_by, updated_date)
						VALUES ('$dataYear','$dataPublisher','$dataAuthor','$file_cover',
          ...
          $mySql   = "SELECT * FROM document_files where document_id='$dataCode' and document_version='$dataVersion'";
          $myQry   = mysqli_query($koneksidb, $mySql)  or die("Error query " . mysqli_error($koneksidb));
          $myData = mysqli_fetch_array($myQry);
          if (mysqli_num_rows($myQry) > 0) {
            $mySql    = "INSERT INTO document_files (document_id, document_version, document_file_name, document_file_title, document_file_ext, 
          document_size, document_bulkid, document_order, updated_by, updated_date)
          (select document_id, '" . $dataVersion . "', document_file_name, document_file_title, document_file_ext, document_size, 
          document_bulkid, document_order, updated_by, updated_date from document_files where document_id='$dataCode' and document_version='$dataVersion')";
            $myQry = mysqli_query($koneksidb, $mySql);
            if (!$myQry)
              throw new Exception("Form gagal diinput. code:d4. " . mysqli_error($koneksidb));
          }
        }
        ...
        commit();
        // echo 'ok';
        echo "<meta http-equiv='refresh' content='0; url=?page=Content-Management-Digital&msg=edit'>";
        exit;
      } catch (Exception $e) {
        rollback();
        // echo "<meta http-equiv='refresh' content='0; url=?page=Content-Management-Digital-Edit&id=$Code&info=" . $e->getMessage() . "'>";
        echo $e->getMessage();
      }
    }
  }
        ...
        <div class="row breadcrumbs-top">
          <div class="col-12">
            ...
            <div class="breadcrumb-wrapper">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="?page=Content-Management-Digital">List Content</a>
                </li>
                <li class="breadcrumb-item active"><a href="#">Edit Content</a>
                </li>
              </ol>
            </div>
          </div>
                    ...
                    <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self" enctype="multipart/form-data">
                      <div class="row">
                        <div class="col-md-2 col-12">
                          <div class="mb-1">
                            <label class="form-label" for="Admin ID">Content ID</label>
                        ...
                        <div class="col-md-2 col-12">
                          ...
                          <input id="tanggal" autocomplete="off" class="dapick form-control" placeholder="YYYY-MM-DD" name="txtExpired" type="text" value="<?php echo $dataExpireDate; ?>" maxlength="20" required="required" autocomplete="off" />
                        </div>
                        ...
                        <div class="col-md-2 col-12">
                          ...
                          <div class="mb-1">
                            <label class="form-label" for="status">Content File</label>
                            <input type="file" id="status" class="form-control" name="txtFileBook" placeholder="Book File" />
                          </div>
                        </div>
                        ...
                        <div class="col-md-12 col-12">
                          <div class="mb-1">
                            ...
                            <textarea name="txtChange" class="form-select" id="" cols="30" rows="2"><?php echo $dataChange; ?></textarea>
                          </div>
                        </div>
                      </div>
